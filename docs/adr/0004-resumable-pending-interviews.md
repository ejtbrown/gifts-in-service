...
Store one pending interview per person in encrypted Aurora PostgreSQL. The row contains the displayed questions and member answers, the latest unapproved proposal when one exists, a broad `LOW`/`MODERATE`/`HIGH` completeness-confidence value, bounded conversation-control state, an optimistic revision, and timestamps. It is accessible only through a valid member session selected for that person; it is never exposed to staff routes, staff search, embeddings, analytics, application logs, traces, or audit records. ADR 0007 refines that conversation-control state into optional follow-up notes plus bounded memory of established facts and member-closed topics.
...
Member interview writes use an optimistic revision so two tabs cannot silently overwrite each other. The server, not the browser, supplies the authoritative transcript and pending conversation-control state to each stateless Bedrock call and supplies the transcript to profile drafting. A correction or new fact after a proposal clears that proposal until a fresh draft is generated, preventing stale prose from being submitted. Browser storage remains prohibited. Confidence, notes, and memory are temporary conversation-control state, not measures of the person's value or suitability, and are deleted with the pending interview.
