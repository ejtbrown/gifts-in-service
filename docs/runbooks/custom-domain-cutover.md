...
1. Obtain church/DNS approval and set `custom_domain_name` plus its managed `route53_zone_id`. Both values are a production deployment precondition. Schedule the change because host-only member and staff cookies do not transfer between the CloudFront hostname and the custom hostname.
