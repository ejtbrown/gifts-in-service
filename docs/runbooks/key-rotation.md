...
- Rotating session or magic-link HMAC keys invalidates outstanding sessions/links and staff trusted-browser cookies. Because the staff device credential is authenticated-encrypted with a domain-separated form of the session key, staff must complete TOTP again after rotation. Schedule a maintenance window, deploy the new secret atomically, and communicate only that users may need to authenticate again.
